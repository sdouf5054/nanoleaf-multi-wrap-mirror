...
MEDIA_AUDIO_IDLE_COUNT = 50         # 연속 N회 idle → 미러링 복귀 (0.1초 × 50 = 5초)
...
class UnifiedEngine(BaseEngine):
    ...
    def _resolve_media_source(self, media_frame):
        ...
        if self._media_detect_state == "audio_idle":
            # 오디오 에너지가 복귀 임계치를 초과하면 Phase 1 재진입
            ...
            return self._media_detect_decision  # "mirror" 유지 (또는 방금 Phase 1으로 전환)
