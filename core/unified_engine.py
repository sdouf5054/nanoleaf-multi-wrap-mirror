...
[미디어 소스 자동판별 v6 — 2-phase + audio idle]
  ...
  Phase 2: 판별 완료 후 지속 모니터링
    ...
    - ★ 앨범아트 상태 + 오디오 idle 연속 N회 → "audio_idle" 상태로 미러링 전환
...
[v6 변경사항]
  - _MEDIA_DEBUG 클래스 변수: False면 상태바에 디버그 메시지 미출력
  - _media_debug() 헬퍼: 디버그 메시지 출력을 한 곳에서 제어
  - _init_audio_monitor(): D=ON, A=OFF, M=ON일 때 경량 오디오 모니터 시작
  - "audio_idle" 상태: Phase 2에서 오디오 무음 감지 → 미러링 복귀
  - 오디오 복귀 시 Phase 1 재진입
...
MEDIA_DETECT_PHASE2_DYNAMIC_COUNT = 20       # 연속 N회 초과 → 미러링 전환 (약 2.0초)
MEDIA_DETECT_PHASE2_STATIC_COUNT  = 25       # 연속 N회 미만 → 앨범아트 전환 (약 2.5초)

# ★ Audio idle 감지 상수 (v6)
MEDIA_AUDIO_IDLE_THRESHOLD = 0.02   # bass+mid+high 합산 에너지 이하면 idle
MEDIA_AUDIO_IDLE_COUNT = 25         # 연속 N회 idle → 미러링 복귀 (0.1초 × 25 = 2.5초)
MEDIA_AUDIO_RESUME_THRESHOLD = 0.05 # audio_idle 상태에서 이 이상이면 오디오 복귀
MEDIA_AUDIO_RESUME_COUNT = 25        # ★ 연속 N회 초과해야 오디오 복귀 (0.1초 × 25 = 2.5초)
...
class UnifiedEngine(BaseEngine):
    ...
    # ★ 미디어 판별 디버그 메시지 출력 제어.
    # False: 상태바에 "[미디어]..." 메시지를 출력하지 않음 (릴리즈).
    # True: 상태바에 상세 판별 로그 출력 (개발/디버깅 시).
    _MEDIA_DEBUG = False
    ...
    def __init__(self, config, **kwargs):
        ...
        # ── ★ 미디어 소스 자동판별 상태 (v6: audio idle 추가) ──
        self._media_detect_state = "idle"       # "idle" | "phase1" | "holding" | "phase2" | "audio_idle"
        ...
        self._media_detect_audio_resume_hits = 0    # ★ audio_idle에서 복귀 연속 hit
    ...
    def _get_audio_energy_total(self):
        ...
        오디오 엔진이 있으면 (audio_enabled=True이든, 미디어 전용 모니터든)
        실시간 에너지를 반환. 없으면 None.
        """
        if self._audio_engine is None:
            return None
        # audio_enabled=True일 때는 _smooth_* 가 매 프레임 갱신되므로 사용
        # audio_monitor_only일 때는 _smooth_*가 갱신 안 되므로 직접 get
    ...
    def _resolve_media_source(self, media_frame):
        ...
        [Phase 2] 판별 완료 후 지속 모니터링
          ...
          현재 "media": ★ 오디오 idle 연속 N회 → "audio_idle" 상태로 미러링 전환
        ...
        # ── 측정 주기 체크 (phase1/phase2/holding 공통) ──
            ...
            if self._media_detect_decision == "media":
                ...
                if mse < MEDIA_DETECT_PHASE2_MSE_THRESHOLD:
                    ...
                    if self._media_detect_phase2_static_hits >= MEDIA_DETECT_PHASE2_STATIC_COUNT:
                        # ★ 오디오도 무음이면 미러링 유지 (불필요한 앨범아트 왕복 방지)
                        total_energy = self._get_audio_energy_total()
    ...
    def _init_mode_resources(self):
        ...
        if ep.audio_enabled:
            ...
            # ★ D=ON, A=OFF, M=ON: 미디어 전용 경량 오디오 모니터
    ...
    def _init_audio_monitor(self):
        ...
        D=ON, A=OFF, M=ON 조합에서 오디오 idle 감지를 위해
        AudioEngine만 시작. 렌더링에는 사용하지 않음.
        CPU 비용 거의 없음 (WASAPI 콜백 + FFT만 동작).
        """
        ...
        try:
            ...
            # 오디오 디바이스 접근 실패 — idle 감지 불가, 하지만 치명적이지 않음
    ...
    def _handle_runtime_param_changes(self, ep, prev_n_zones, prev_zone_weights,
        ...
        if (ep.media_source_override == "auto"
            ...
            # 결과 반전 후 holding로 진입 — 3초 유지 뒤 phase2로 자동 전환
            if self._media_detect_decision == "media":
                self._media_detect_decision = "mirror"
            else:
                self._media_detect_decision = "media"
