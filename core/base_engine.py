...
class BaseEngine(QThread):
    ...
    def __init__(self, config, audio_device_index=None):
        ...
        self._current_params = EngineParams(
            ...
            mirror_n_zones=config.get("mirror", {}).get("zone_count", N_ZONES_PER_LED),
        )
    ...
    def _rebuild_pipeline(self):
        ...
        mirror_cfg_copy = {
            ...
            "smoothing_factor": ep.smoothing_factor,
        }
    ...
    def _handle_display_change(self):
        ...
        try:
            ...
            self.status_changed.emit(
                f"디스플레이 변경 반영 완료 "
                f"({self._active_w}×{self._active_h}, grid {new_grid_cols}×{new_grid_rows})"
            )
