...
def _build_log_bands(n_bands, freq_min, freq_max, fft_freqs):
    ...
    로그 스케일은 저음에 더 많은 밴드를 할당하고
    고음은 넓은 범위를 묶어서, 인간 청각 특성에 맞춥니다.
...
class AudioEngine:
    ...
    두 가지 출력을 동시 제공:
    1. 3밴드 (bass/mid/high) — pulse 모드용, 대역별 감도 적용
    2. N밴드 스펙트럼 (기본 16) — spectrum/wave 모드용, 로그 스케일

    Args:
        device_index: WASAPI Loopback 디바이스 인덱스 (None=자동)
        block_size: FFT 윈도우 크기
        n_bands: 세밀 스펙트럼 밴드 수
        sensitivity: 기본 감도 (bass/mid/high 개별 설정 가능)
        smoothing: 내부 EMA 스무딩 (노이즈 제거용, 고정)
    """
    ...
    def __init__(self, device_index=None, block_size=DEFAULT_BLOCK_SIZE,
        ...
        # ── FFT 사전 계산 ──
        fft_freqs = np.fft.rfftfreq(block_size, 1.0 / self._sample_rate)

        # 3밴드 마스크
        self._bass_mask = (fft_freqs >= BAND_BASS[0]) & (fft_freqs < BAND_BASS[1])
        self._mid_mask = (fft_freqs >= BAND_MID[0]) & (fft_freqs < BAND_MID[1])
        self._high_mask = (fft_freqs >= BAND_HIGH[0]) & (fft_freqs < BAND_HIGH[1])

        # N밴드 (로그 스케일)
        self._band_bins = _build_log_bands(n_bands, FREQ_MIN, FREQ_MAX, fft_freqs)
        ...
        # ── 밴드별 AGC ──
        self._agc_3band = np.full(3, 0.01, dtype=np.float64)   # [bass, mid, high]
    ...
    def _audio_callback(self, in_data, frame_count, time_info, status):
        try:
            ...
            # N밴드에도 위치별 감도 적용 (저음 밴드→bass_sens, 고음→high_sens)
            # 로그 스케일이라 대략 1/3씩 bass/mid/high
